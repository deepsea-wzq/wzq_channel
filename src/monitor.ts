...
// ── Device Identity 辅助函数（兼容新版 openclaw gateway ≥ 2026.3.7）──
// 新版 gateway 要求 connect 时携带签名的 device identity，否则会清空 scopes。
// 以下函数兼容新旧版本：有 device identity 就带上，没有则回退到旧行为。
...
type DeviceIdentity = { deviceId: string; publicKeyPem: string; privateKeyPem: string } | null;

/**
 * 加载或创建设备身份（Ed25519 密钥对），存储在 ~/.openclaw/state/identity/device.json
 * 兼容 openclaw 官方 CLI 使用的同一个 device identity 文件。
 */
function loadOrCreateDeviceIdentity(): DeviceIdentity {
  try {
    // openclaw 默认路径：~/.openclaw/identity/device.json
    // (stateDir 在标准安装中就是 ~/.openclaw)
    const openclawDir = path.join(process.env.HOME || '~', '.openclaw');
    const identityFile = path.join(openclawDir, 'identity', 'device.json');
    ...
    console.warn(`[gateway] 加载/创建 device identity 失败: ${err}`);
    return null;
  }
}

/**
 * 构建 device auth 签名载荷 (v3 格式，兼容新版 gateway)
 */
function buildDeviceAuthPayloadV3(opts: {
  ...
  return [
    ...
    opts.token ?? '', opts.nonce, opts.platform, '',
  ].join('|');
}

/**
 * 通用 gateway RPC 调用
 * 连接 gateway WS → challenge-response 认证（带 device identity）→ 发送指定方法 → 返回 payload
 */
    ...
    gwWs.on('message', (data: any) => {
      try {
        ...
        if (msg.type === 'event' && msg.event === 'connect.challenge') {
          ...
          // 构建 device identity（新版 gateway 需要，旧版会忽略）
          let device: any = undefined;
          if (deviceIdentity && nonce) {
            const signatureToken = gatewayToken || null;
